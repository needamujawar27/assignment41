...
export const Teachers=()=>{
    ...
    const getData=async()=>{
    const result= await axios.get("http://localhost:3060/teachers");
    setData(result.data);
    }
    useEffect(()=>{
     getData()
...
<Grid container spacing={2} sx={{marginTop:"20px"}}>
    ...
    <Typography variant="h3" textAlign={"center"}>Teachers</Typography>
    </Grid>
{
    data.map((item)=>{
        return(
         <Grid item xs={6}>
            <Card sx={{bgcolor:"black",margin:"10px",color:"white",boxShadow:"5px 5px 5px darkolivegreen",borderRadius:"500%",textAlign:"center"}}>
              ...
              <Typography variant="h5" ><span>Subject:</span>{item.subject}</Typography>
            </CardContent>
        </Card>
